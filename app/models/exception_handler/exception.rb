...
##
# Module of exception_handler gem inserted here to force some overrides
#
module ExceptionHandler
  ...
  BOTS = %w[Baidu Gigabot Googlebot libwww-per lwp-trivial msnbot SiteUptime Slurp Wordpress ZIBB ZyBorg Yandex Jyxobot
            Huaweisymantecspider ApptusBot].freeze

  # => Attributes
  # => Determine schema etc
  ATTRS = %i[class_name status message trace target referer params user_agent response app_name ip
  ...
  self::Exception =
    ...
    after_initialize -> (e) {
 ExceptionHandler::ExceptionMailer.new_exception(e).deliver }, if: :email? # => see bottom of file
    ...
    # => Params
    def params
      request.params.except('utf8').except('authenticity_token').except_nested('password').inspect
    end
    ...
    def email?
      return false if status.to_s.first == '4'
